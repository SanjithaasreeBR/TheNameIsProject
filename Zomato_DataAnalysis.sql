...
drop table if exists Zomato.customer;
...
# 1) Find Total number of Customer

Select Count(1) as Total_No_of_Customer from customer;

# 2) Count of Customer who never ordered

Select customer_id, customer_name from customer 
where customer_id Not In (Select customer_id from order_detail);

# 3) Total Sales amout of Each Dish in Zomato in Descending 

Select f.food_id,f.food_name, round(sum(f.price_per_unit),2) as Average_Sales_of_Each_Dishes 
from order_food fo
join foods f on fo.food_id = f.food_id
group by f.food_id order by Average_Sales_of_Each_Dishes desc;

#4) Top 5 restaurants Based on No_of_Order for October Month
Select r.restaurant_id,r.restaurant_name,count(od.order_id) as Total_OrderReceived from restaurant r
join 
order_detail od on r.restaurant_id = od.restaurant_id
where monthname(od.order_time) = "OCTOBER"
Group by r.restaurant_id order by Total_OrderReceived desc limit 5;

#5) Monthly Revenue sales Analysis

Select r.restaurant_id,r.restaurant_name,sum(f.price_per_unit) as Monthly_Revenue_By_ZomatoSale from order_food fo 
join restaurant r 
join foods f join order_detail od  
on  r.restaurant_id = fo.restaurant_id and fo.food_id = f.food_id  and od.order_id=fo.order_id 
where monthname(od.order_time) = "OCTOBER"
group by r.restaurant_id order by r.restaurant_id ;

#6) Find List  of Restaurant which has Repeated customers and its no of times of Purchase
with cte1 as ( 
Select od.restaurant_id,r.restaurant_name,od.customer_id, row_number() over (partition by od.restaurant_id,od.customer_id order by r.restaurant_id,od.customer_id) as Ranks from restaurant r join order_detail od 
on r.restaurant_id=od.restaurant_id ),
cte2 as (select restaurant_id,restaurant_name,customer_id,count(ranks) as Customer_Reoccurences from cte1 group by restaurant_id,restaurant_name,customer_id),
cte3 as (Select restaurant_id,restaurant_name,customer_id,max(Customer_Reoccurences ) as Loyal_Customer_Vist from cte2 
group by restaurant_id ,restaurant_name,customer_id order by  restaurant_id ,Loyal_Customer_Vist desc ) 
Select restaurant_id,restaurant_name,customer_id as Loyal_Customer,Loyal_Customer_Vist from cte3 order by Loyal_Customer_Vist desc Limit 1;

# 7) Month over Month Analysis of Restaurant sales 

with cte1 as (Select r.restaurant_id,r.restaurant_name, monthname(od.order_Time) as Months , month(od.order_Time) as month_No, sum(f.price_per_unit) as Monthly_Revenue_By_Zomato  from order_food fo 
join restaurant r 
join foods f join order_detail od  
on  r.restaurant_id = fo.restaurant_id and fo.food_id = f.food_id  and od.order_id=fo.order_id 
group by r.restaurant_id,Months,month_No order by r.restaurant_id,month_No ),
cte2 as (Select * , row_number() over (partition by  restaurant_id ),
(case
when  Monthly_Revenue_By_Zomato > lag(Monthly_Revenue_By_Zomato ) over ( order by restaurant_id,restaurant_name,month_No ) then  concat(Months , "-" ," Has Progressed in Monthly Revenue" )
else 
concat(Months , " has " ,Monthly_Revenue_By_Zomato)
end) as Results from cte1
)
select restaurant_id,restaurant_name,Months,Results from cte2;

#8) Each Restaurants with Most Sold Food item

with cte1 as 
(select r.restaurant_id,r.restaurant_name , f.food_name,count(f.food_id) as Ordered_frequency 
from restaurant r join foods f join order_food fo on r.restaurant_id=fo.restaurant_id and f.food_id=fo.food_id
group by r.restaurant_id,r.restaurant_name,f.food_name 
order by  r.restaurant_id,Ordered_frequency desc),
cte2 as 
(select * , row_number() over (partition by restaurant_id,restaurant_name order by Ordered_frequency desc) as RowNum from cte1)
(select * from cte2 where rownum = 1)
;

#9) Restaurant and its Most Ordered items
...
cte2 as 
(select * from cte1)
select restaurant_id, restaurant_name, group_concat(food_name) as Restaurant_Spl_Menu  from cte2 group by restaurant_id, restaurant_name;
